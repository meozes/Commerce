...
public class OrderOutboxConsumer { // 아웃박스 상태 변경
    private final OrderOutboxRepository outboxRepository;

    @KafkaListener(topics = "order-data-platform", groupId = "order-publisher-group")
    @Transactional
    public void consumeOrderDataPlatform(OrderSendEvent event) {
        try {
            OrderOutbox outbox = outboxRepository.findByOrderId(event.getOrderId())
                    .orElseThrow(() -> new IllegalStateException("아웃박스에 해당 데이터 없음 " + event.getOrderId()));
